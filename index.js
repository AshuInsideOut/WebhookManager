...
const consoleManager = require('@ashuinsideout/console-manager');
const { commandManager } = consoleManager;

commandManager.addCommand({
  command: 'stop',
  handler: async () => {
    console.log(`Thanks for using the AshuInsideOut's Webhook Manager!`);
    process.exit(0);
  },
...
commandManager.addCommand({
  ...
  handler: async (command, args) => {
    ...
    const res = webhooks[webhookId].match(
    ...
    if (content.embed) {
      if (
        ...
        Array.isArray(content.embed.description)
      ) {
        let description = '';
        content.embed.description.forEach(
...
console.log('\n====================================');
console.log(`Welcome to AshuInsideOut's Webhook Manager`);
console.log('====================================');
...
function generateEmbed(obj) {
  ...
  if (!newObject.color)
    newObject.color = Utils.variables.config.EmbedColors.Default;
  if (typeof newObject.footer === 'string')
    newObject.footer = { text: newObject.footer };
  if (typeof newObject.author === 'string')
    newObject.author = { name: newObject.author };
  if (typeof newObject.thumbnail === 'string')
    newObject.thumbnail = { url: newObject.thumbnail };
  if (typeof newObject.image === 'string')
    newObject.image = { url: newObject.image };
  if (typeof newObject.timestamp === 'string')
    newObject.timestamp = parseInt(newObject.timestamp);
  if (newObject.description)
    newObject.description = newObject.description.toString().slice(0, 2048);
